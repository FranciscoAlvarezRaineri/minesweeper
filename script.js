...
function titleCreate() {
  ...
  document.getElementById("menu").addEventListener("change", function () {
    setDificultySize(event);
    return reset();
  });
  title.face.addEventListener("click", function () {
    reset();
  });
  title.instructions.addEventListener("click", function () {
    showInstructions();
  });
  title.time = 0;
  title.timeX;
...
function setCustom() {
  xInit = window.prompt("Casillas horizontales", 20);
  y = window.prompt("Casillas verticales", 12);
  minesRatio = window.prompt(
    "Cantidad de bombas en un ratio. Ej: 4 equivale a 1 mina cada 4 casillas",
    4
  );
  if (minesRatio <= 2) {
  ...
  if (minesRatio > 8) {
    alert(
      `No son suficientes minas, sería demasiado fácil. Elige otros valores.`
    );
    return setCustom();
  }
}
...
function showInstructions() {
  alert(
    "Un click sobre la casilla revela su contenido. El numero revelado indica la cantidad de bombas aledañas a la casilla. Si clickeas en una bomba pierdes. Un click sostenido sobre una casilla sin revelar, la marca como una bomba. Un click sostenido sobre una casilla revelada, revela las aledañas que no esten marcadas como bombas."
  );
}
// selecciona la dificultad y el tamaño
function setDificultySize(event) {
...
class Button {
  constructor(y, x) {
    ...
    this.element.id = `${y.toString().padStart(2, "0")}${x
      .toString()
      .padStart(2, "0")}`;
...
function setMines() {
  ...
  while (i < amountMines) {
    ...
    if (
      ...
      board.arrButtons[location].element.value != "reserved"
    ) {
      board.arrButtons[location].element.value = "💣";
      i++;
    }
...
function findAdjacents(button) {
  ...
  function addButton(tempPositionX, tempPositionY) {
    if (
      tempPositionY != button.positionY ||
      tempPositionX != button.positionX
    ) {
      buttonTemp = selectButtonFromId(
        `${tempPositionY.toString().padStart(2, "0")}${tempPositionX
          .toString()
          .padStart(2, "0")}`
      );
  ...
  function selectButtonFromId(buttonId) {
    return board.arrButtons.filter((button) => {
      return button.element.id === buttonId;
    })[0];
  }
  return adjacents;
}
...
function revealValue(button) {
  ...
  if (
    board.arrButtons.filter((button) => {
      return button.revealed === false;
    }).length -
      amountMines ===
    0
  ) {
